...
export type AdminKind = "dashboard"|"roster"|"schedule"|"seasons"|"stats"|"sponsors"|"store"|"analytics";
const REQUIRED: Partial<Record<AdminKind,FeatureKey>> = { seasons:"seasons",stats:"stats",sponsors:"sponsors",store:"store",analytics:"analytics" };
...
function AdminContent({ kind }: { kind: AdminKind }) {
  const store = useMockData(); const { tier } = useTier(); const hasSeasons = useHasFeature("seasons"); const hasStats = useHasFeature("stats");
